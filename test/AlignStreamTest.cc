...
struct infilenames
{
    ...
    const char* phylip_input_data;
    infilenames()
        : input_data{ "data/CG15644-Z.aln" }, phylip_input_data{
              "data/phylip_input.txt"
          }
    {
    }
};
...
BOOST_FIXTURE_TEST_SUITE(AlignStreamTest, infilenames)
...
BOOST_AUTO_TEST_CASE(from_vector3)
{
    std::vector<Sequence::Fasta> vf
        = { Sequence::Fasta("seq1", "ATG"), Sequence::Fasta("seq2", "ATC") };
    BOOST_REQUIRE_NO_THROW(clustal c(std::move(vf)); BOOST_CHECK(vf.empty());
...
BOOST_AUTO_TEST_CASE(from_vector4)
{
    std::vector<Sequence::Fasta> vf
        = { Sequence::Fasta("seq1", "ATG"), Sequence::Fasta("seq2", "ATC") };
    BOOST_REQUIRE_NO_THROW(phylip c(std::move(vf)); BOOST_CHECK(vf.empty());
                           BOOST_REQUIRE_EQUAL(c.size(), 2););
}
...
BOOST_AUTO_TEST_CASE(copy_construct1)
{
    ...
    BOOST_REQUIRE_NO_THROW(clustal c2(c);
                           ...
                           for (decltype(c.size()) i = 0; i < c.size();
                                ++i) { BOOST_REQUIRE(c[i] == c2[i]); });
}
...
BOOST_AUTO_TEST_CASE(should_throw1)
{
    ...
    std::vector<Sequence::Fasta> vf
        = { Sequence::Fasta("seq1", "ATG"), Sequence::Fasta("seq2", "ATCAA") };

    BOOST_REQUIRE_THROW(clustal c(vf), std::runtime_error);
    BOOST_REQUIRE_THROW(phylip c(vf), std::runtime_error);
    BOOST_REQUIRE_THROW(clustal c(std::vector<Sequence::Fasta>(
                            { Sequence::Fasta("seq1", "ATG"),
                              Sequence::Fasta("seq2", "ATCAA") })),
                        std::runtime_error);
    BOOST_REQUIRE_THROW(clustal c(std::move(std::vector<Sequence::Fasta>(
                            { Sequence::Fasta("seq1", "ATG"),
                              Sequence::Fasta("seq2", "ATCAA") }))),
                        std::runtime_error);
}
...
BOOST_AUTO_TEST_CASE(should_throw3)
{
    ...
    std::vector<Sequence::Fasta> vf
        = { Sequence::Fasta("seq1", "ATG"), Sequence::Fasta("seq2", "ATCAA") };
    clustal c;
    BOOST_REQUIRE_THROW(c.assign(vf.begin(), vf.end()), std::runtime_error);
    //And make sure it works with c++11-style iterator fxns
    BOOST_REQUIRE_THROW(c.assign(vf.cbegin(), vf.cend()), std::runtime_error);
}
