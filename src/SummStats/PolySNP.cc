...
namespace Sequence
{
    ...
    {
        inline bool
        operator()(const std::string &l, const std::string &r) const
        {
            // use Sequence::Different to prevent missing sites
            // causing 2 sequences to be labelled as distinct
            // return (  Different(l,r) &&
            // std::lexicographical_compare(l.begin(),l.end(),r.begin(),r.end(),lt_nocase())
            // );
            return (Different(l, r)
                    && std::lexicographical_compare(
                           ...
                           [](const char &__a, const char __b) {
                               return (std::toupper(
                                           static_cast<unsigned char>(__a))
                                       < std::toupper(
                                             static_cast<unsigned char>(__b)));
                           }));
        }
    };
    _PolySNPImpl::_PolySNPImpl(const Sequence::PolyTable *data,
                               const bool &haveOutgroup,
                               const unsigned &outgroup, const bool &totMuts)
        : _data(data), _nsites(data->numsites()),
          _nsam(unsigned(data->size())), _outgroup(outgroup),
          _haveOutgroup(haveOutgroup), _totMuts(totMuts),
          _totsam(unsigned(data->size())), _DVK(0), _DVH(1.0),
          _counted_singletons(false), _know_pi(false), _CalculatedDandV(false),
          _pi(0.), _singletons(0), _walls_Bprime(0), _NumPoly(0), _walls_B(0.),
          _walls_Q(0.), _calculated_wall_stats(false),
          _counts(std::vector<Sequence::stateCounter>(
              _nsites, Sequence::stateCounter('-'))),
    ...
    */
    ...
    PolySNP::PolySNP(const Sequence::PolyTable *data, bool haveOutgroup,
        ...
        : rep(std::unique_ptr<_PolySNPImpl>(
              new _PolySNPImpl(data, haveOutgroup, outgroup, totMuts)))
    /*!
      ...
      \param outgroup if \a haveOutgroup is \c true, \a outgroup is the index
      of that sequence in data
      \param totMuts if true (the default) use the total number of inferred
      mutations,
      otherwise use the total number of polymorphic sites in calculations
      \note this constructor allocates a pointer to an implementation class,
      which
    ...
    */
    ...
    /*!
      Calculated here as the sum of 1.0 - sum of site homozygosity accross
      sites.\n
      \f[
      \widehat\theta_\pi=\sum_{i=1}^{i=S}(1-\sum_{j=1}^{j=4}\frac{k_{j,i}
      \times (k_{j,i}-1)}{n_i \times (n_i - 1)});k_{j,i}>0
      \f]
      Where \f$S\f$ is the number of segregating sites, \f$k_{j,i}\f$ is the
      number of occurences of the \f$j_{th}\f$
      character state at site \f$i\f$, and \f$n_i\f$ is the sample size at site
      \f$i\f$.
      Calculating the statistic in this manner makes it easy to generalize to
      an arbitrary number
      ...
      \f]\n
      Also equivalent to mean pairwise differences, but that's slow to
      calculate.\n
      \n
      If there is missing data (indicated by 'N' characters), the sample size
      is reduced for that site.  For example, if the data for the \f$i_{th}\f$
      site is:\n
      A\n
      A\n
      A\n
      ...
      N\n
      G\n
      Then ThetaPi is calculated for that site as if the sample size were 4
      (not 6),
    ...
    */
                    ...
                    { // iterate over sites
                        if (rep->_counts[i].gap == 0
                            && rep->_counts[i].nStates() > 1)
                            {
                                ...
                                samplesize
                                    -= rep->_counts[i].n; // adjust sample size
                                                          // for missing data
                                if (samplesize > 1)
                                    {
                                        double SSH
                                            = 0.0; // sum of site homozygosity
                                        double denom
                                            = (double(samplesize)
                                               * (double(samplesize) - 1.0));
                                        SSH += (rep->_counts[i].a > 0)
    ...
    /*!
      ...
      or the number of mutations on the genealogy and \f$n_i\f$ is the sample
      size
      at site i. If totMuts == 1,
      the number of mutations is used, else the number ofsegregating sites is
      used.
      \warning Statistic undefined if there are untyped SNPs.  In the presence
      of
      missing data, ThetaW is calculated as the sum (over all segregating
      sites)
      of 1/a_sub_n, where a_sub_n is the denominator of ThetaW, using the
      number
      of alleles without missing data as the sample size.  More formally, the
      routine returns a calculation base on an unweighted sample size
      adjustment accross
      sites.
    */
    {
    ...
    /*!
      ...
      This statistic is problematic in general to calculate when there are
      multiple hits.
      The test requires that the ancestral state (inferred from the outgroup)
      still be segregating
      in the ingroup.  If that is not true, the site is skipped.\n

      If there are >= 2 derived states inferred, a "missing data" approach is
      taken.\n
      For example:\n
      Outgroup : \n
      A\n
      ...
      (as the 0,1 coding of the data refers to ancestral and derived,
      respectively).
    */
    {
        assert(rep->_preprocessed);
        ...
        for (unsigned i = 0; i < rep->_nsites; ++i)
            { // iterate over sites
                if (rep->_derivedCounts[i].second.gap == 0)
                    {
                        unsigned samplesize
                            = rep->_totsam; // sample size per site
                        ...
                        unsigned ancestralCounts
                            = samplesize - sumDerCounts
                              - rep->_derivedCounts[i].second.n;

                        // if the ancestral state is not missing data, and
                        // the sum of the derived counts + and missing data in
                        // the ingroup
                        // does not equal the sample size, then the ancestral
                        // state is present
                        // at least once in the ingroup, and anc_is_present =
                        // true
                                ...
                                if (numDer == 1)
                                    { // simple if there is only one derived
                                        ...
                                        double denom
                                            = (double(samplesize)
                                               * (double(samplesize) - 1.0));
                                        H += (rep->_derivedCounts[i].second.a
                                              > 0)
                                                 ? (2.0
                                                    * pow(double(
                                                              rep->_derivedCounts
                                                       ...
                                                       / denom
                                                 : 0.;
                                        H += (rep->_derivedCounts[i].second.g
                                              > 0)
                                                 ? (2.0
                                                    * pow(double(
                                                              rep->_derivedCounts
                                                       ...
                                                       / denom
                                                 : 0.;
                                        H += (rep->_derivedCounts[i].second.c
                                              > 0)
                                                 ? (2.0
                                                    * pow(double(
                                                              rep->_derivedCounts
                                                       ...
                                                       / denom
                                                 : 0.;
                                        H += (rep->_derivedCounts[i].second.t
                                              > 0)
                                                 ? (2.0
                                                    * pow(double(
                                                              rep->_derivedCounts
                                              ...
                                              > 0)
                                                 ? (2.0
                                                    * pow(double(
                                                       ...
                                                       / denom
                                                 : 0.;
                                        H += (rep->_derivedCounts[i].second.one
                                              > 0)
                                                 ? (2.0
                                                    * pow(double(
                                                              rep->_derivedCounts
                                    ...
                                    { // use a "missing data" scheme if there
                                        ...
                                        unsigned config[2];
                                        unsigned k = 0;
                                        if (rep->_derivedCounts[i].second.a
                                            > 0)
                                            {
                                                config[k++]
                                                    = rep->_derivedCounts[i]
                                                          .second.a;
                                            }
                                        if (rep->_derivedCounts[i].second.g
                                            > 0)
                                            {
                                                config[k++]
                                                    = rep->_derivedCounts[i]
                                        ...
                                        if (rep->_derivedCounts[i].second.c
                                            > 0)
                                            {
                                                config[k++]
                                                    = rep->_derivedCounts[i]
                                        ...
                                        if (rep->_derivedCounts[i].second.t
                                            > 0)
                                            {
                                                config[k++]
                                                    = rep->_derivedCounts[i]
                                        ...
                                        if (rep->_derivedCounts[i].second.zero
                                            > 0)
                                            {
                                                config[k++]
                                                    = rep->_derivedCounts[i]
                                        ...
                                        if (rep->_derivedCounts[i].second.one
                                            > 0)
                                            {
                                                config[k++]
                                                    = rep->_derivedCounts[i]
    ...
    /*!
      Calculate Theta ( = 4Nu) from site homozygosity, corresponding to
      equation 1 in
      Thornton and Andolfatto (Genetics) "Approximate Bayesian Inference
      reveals evidence
      for a recent, severe, bottleneck in a Netherlands population of
      ...
      This keeps the expectation of the statistic equal to \f$\theta\f$,
      and uses the correct number of derived mutations ovserved in the data.
      \note For sequence data, an outgroup is required.  This requirement is
      checked by assert()
    ...
    */
    {
        ...
        for (unsigned i = 0; i < rep->_nsites; ++i)
            { // iterate over sites
                if (rep->_counts[i].nStates() > 1 && rep->_counts[i].gap == 0)
                    ++npoly;
            }
        return npoly;
    }
    ...
    */
    {
        ...
        for (unsigned i = 0; i < rep->_nsites; ++i)
            { // iterate over sites
                unsigned nstates = (rep->_counts[i].gap == 0)
                                       ? rep->_counts[i].nStates()
                                       : 0;
                if (nstates > 1)
                    nmut += nstates - 1;
            }
    ...
    /*!
      \return number of polymorphisms that appear once in the data, without
      respect to ancestral/derived
    */
    {
        assert(rep->_preprocessed);
        unsigned nsing = 0;
                ...
                if (rep->_counts[i].gap == 0 && nstates > 1)
                    {
                        ...
                        if (nsam == 2 && nstates == 2) // if n = 2 and there
                                                       // are 2 states, there
                                                       // must be 1 singleton
                            curr_nsing = 1;
                        else
                            {
                                curr_nsing
                                    += (rep->_counts[i].a == 1) ? 1u : 0u;
                                curr_nsing
                                    += (rep->_counts[i].g == 1) ? 1u : 0u;
                                curr_nsing
                                    += (rep->_counts[i].c == 1) ? 1u : 0u;
                                curr_nsing
                                    += (rep->_counts[i].t == 1) ? 1u : 0u;
                                curr_nsing
                                    += (rep->_counts[i].zero == 1) ? 1u : 0u;
                                curr_nsing
                                    += (rep->_counts[i].one == 1) ? 1u : 0u;
                            }
                    }
                nsing += curr_nsing;
            }
    ...
    /*!
      ...
      \note For sequence data, an outgroup is required. Will return
      SEQMAXUNSIGNED if that is not the case.
    */
    {
        if (!rep->_haveOutgroup)
            return SEQMAXUNSIGNED;
        assert(rep->_preprocessed);
        unsigned next = 0;
        for (unsigned i = 0; i < rep->_nsites; ++i)
            { // iterate over sites
                ...
                if (rep->_derivedCounts[i].first == true
                    && rep->_derivedCounts[i].second.gap == 0)
                    {
                        ...
                        curr_next += (rep->_derivedCounts[i].second.a == 1)
                                         ? 1u
                                         : 0u;
                        curr_next += (rep->_derivedCounts[i].second.g == 1)
                                         ? 1u
                                         : 0u;
                        curr_next += (rep->_derivedCounts[i].second.c == 1)
                                         ? 1u
                                         : 0u;
                        curr_next += (rep->_derivedCounts[i].second.t == 1)
                                         ? 1u
                                         : 0u;
                        curr_next += (rep->_derivedCounts[i].second.zero == 1)
                                         ? 1u
                                         : 0u;
                        curr_next += (rep->_derivedCounts[i].second.one == 1)
                                         ? 1u
                                         : 0u;
                    }
    ...
    */
    ...
    double
    PolySNP::Hprime(const bool &likeThorntonAndolfatto) const
    /*!
      \return ThetaPi-ThetaH/(~Var(ThetaPi-ThetaH)).  This corresponds to
      Equation 5 in
      Thornton and Andolfatto (Genetics) "Approximate Bayesian Inference
      reveals evidence
      for a recent, severe, bottleneck in a Netherlands population of
      Drosophila melanogaster" and Equation 13 of Zeng et al. (2006) Genetics
      1431-1439
      \param likeThorntonAndolfatto The calculation of H' requires calculation
      of
      \f$\theta^2\f$.  In Thornton and Andolfatto, we simply used
      \f$\widehat\theta_W^2\f$,
      which is slightly biased.  By default, this function calculates
      \f$\theta^2=\frac{S(S-1)}{a_n^2+b_n}\f$, unless this bool is set to
      false,
    ...
    */
    {
        ...
        double thetasq = (likeThorntonAndolfatto == false)
                             ? S * (S - 1) / (a * a + b)
                             : theta * theta;

        double vThetal = (rep->_totsam * theta) / (2.0 * (rep->_totsam - 1.0))
                         + (2.0 * pow(rep->_totsam / (rep->_totsam - 1.0), 2.0)
                                * (b_n_plus1 - 1.0)
                            - 1.0)
                               * thetasq;
        ...
        double vPi
            = (3.0 * rep->_totsam * (rep->_totsam + 1.0) * theta
               + 2.0 * (rep->_totsam * rep->_totsam + rep->_totsam + 3.0)
                     * thetasq)
              / (9 * rep->_totsam * (rep->_totsam - 1.0));

        double cov
            = ((rep->_totsam + 1.0) / (3.0 * (rep->_totsam - 1.0))) * theta
              + ((7.0 * rep->_totsam * rep->_totsam + 3.0 * rep->_totsam - 2.0
                  - 4.0 * rep->_totsam * (rep->_totsam + 1.0) * b_n_plus1)
    ...
    /*!
      \warning statistic undefined if there are untyped SNPs
      \return Denominator of Tajima's D, or nan if there are no polymorphic
      sites
    */
    {
        assert(rep->_preprocessed);
        ...
        b2 = (2.0 * (pow(rep->_totsam, 2.0) + rep->_totsam + 3.0))
             / (9.0 * rep->_totsam * (rep->_totsam - 1.0));
        c1 = b1 - 1.0 / a1;
        c2 = b2 - (rep->_totsam + 2.0) / (a1 * rep->_totsam)
             + a2 / pow(a1, 2.0);
        e1 = c1 / a1;
        e2 = c2 / (pow(a1, 2.0) + a2);
    ...
    /*!
      Calculate the number of haplotypes in the sample, and haplotype
      diversity.
        ...
        if (!(rep->_CalculatedDandV))
            {
                ...
                if (rep->_data->size() > 0)
                    {
                        // step 1 : determine which sequences are unique in the
                        // data,
                        ...
                        if (rep->_haveOutgroup)
                            {
                                unique_haplotypes.insert(rep->_data->begin(),
                                                         rep->_data->begin()
                                                             + rep->_outgroup);
                                unique_haplotypes.insert(
                                    rep->_data->begin() + rep->_outgroup + 1,
                                    rep->_data->end());
                            }
                        else
                            {
    ...
    */
    ...
    unsigned
    PolySNP::WallsBprime(void) const
    /*!
      \return Wall's B' Statistic. Wall, J. (1999) Genetical Research
      74, pp 65-79
      @author Kevin Thornton
    ...
    */
    ...
    /*!
      Total variance of mean pairwise differences. Tajima in Takahata/Clark
      book, (13).
      \warning statistic undefined if there are untyped SNPs
    */
    {
        if (rep->_data->empty() || !NumPoly())
            return std::numeric_limits<double>::quiet_NaN();
        double Pi = ThetaPi();
        double variance = 3.0 * rep->_totsam * (rep->_totsam + 1.0) * Pi
                          + 2.0 * (pow(rep->_totsam, 2.0) + rep->_totsam + 3.0)
                                * pow(Pi, 2.0);
    ...
    /*!
      Stochastic variance of mean pairwise differences. Tajima in
      Takahata/Clark book, (14).
      \warning statistic undefined if there are untyped SNPs
    */
    {
        if (rep->_data->empty() || !NumPoly())
        ...
        double Pi = ThetaPi();
        double variance
            = (3.0 * pow(rep->_totsam, 2.0) - 3.0 * rep->_totsam + 2.0) * Pi
    ...
    */
    {
        if (rep->_data->empty() || !NumPoly())
            return std::numeric_limits<double>::quiet_NaN();
        double Pi = ThetaPi();
        double variance = 2.0 * (3.0 * rep->_totsam - 1.0) * Pi
                          + 2.0 * (2.0 * rep->_totsam + 3.0) * pow(Pi, 2.0);
        variance /= (11.0 * pow(rep->_totsam, 2.0) - 7.0 * rep->_totsam + 6.0);
        return (variance);
    }
    ...
    */
    ...
    /*!
      \return The Fu and Li (1993) D statistic, or nan if there are no
      polymorphic sites.
      \note For sequence data, an outgroup is required.  This requirement is
      checked by assert()
      \warning statistic undefined if there are untyped SNPs
    */
    {
        assert(rep->_preprocessed);
        //    assert(rep->_haveOutgroup == true);
        if (rep->_NumPoly == 0 || !rep->_haveOutgroup)
            return std::numeric_limits<double>::quiet_NaN();
    ...
    /*!
      \return Fu and Li (1993) F statistic, or nan if there are no polymorphic
      sites
    ...
    */
    {
        ...
        double vF
            = c
              + 2.0 * (pow(rep->_totsam, 2.0) + rep->_totsam + 3.0)
                    / (9.0 * rep->_totsam * (double(rep->_totsam - 1.0)));
        vF -= (2.0 / (rep->_totsam - 1.0));
        vF /= (pow(a, 2.0) + b);

        double uF
            = 1.0
              + (rep->_totsam + 1.0) / (3.0 * (double(rep->_totsam - 1.0)));
    ...
    */
    {
        ...
        double uD = (rep->_totsam / (rep->_totsam - 1.0))
                        * (a - (rep->_totsam / (rep->_totsam - 1.0)))
                    - vD;

        double DStar = (rep->_totsam / (rep->_totsam - 1.0)) * NumMut
                       - a * double(Singletons);
    ...
    */
    {
        ...
        // Simonsen et al.  (1995) Genetics 141: 413, eqn A5
        double vF = 2.0 * pow(rep->_totsam, 3.0)
                    + 110.0 * pow(rep->_totsam, 2.0) - 255.0 * rep->_totsam
                    + 153.0;
        ...
        double uF = (4.0 * pow(rep->_totsam, 2.0) + 19.0 * rep->_totsam + 3.0
        ...
        double FStar
            = Pi
              - (((rep->_totsam - 1.0) / rep->_totsam)) * double(Singletons);
    ...
    */
    ...
    /*!
      ...
      1 , & when\ n = 2 \\
      \frac{2 \times (n \times a_n - 2 \times (n-1))}{(n-1) \times (n-2)}, &
      when\ n > 2 \\
      \end{array}\right.\
      \f]
      \warning statistic undefined if there are untyped SNPs
    */
    ...
    /*!
      \f[\ d_n=\frac{2}{n-1} \times (1.5 - \frac{2 \times a_{n+1}}{n-2} -
      \frac{1}{n})\ \f]
      \warning statistic undefined if there are untyped SNPs
    */
    { // from Fu and Li 93
        ...
        d += (2.0 / (rep->_totsam - 1.0))
             * (1.5 - ((2.0 * a_n_plus1 - 3.0) / (rep->_totsam - 2.0))
                - 1.0 / rep->_totsam);
        return d;
    }
    ...
    */
    {
        ...
        if (rep->_NumPoly == 0)
            return std::numeric_limits<double>::quiet_NaN();
        return (Recombination::HudsonsC(rep->_data, rep->_haveOutgroup,
                                        rep->_outgroup));
    }
    ...
    */
        ...
        for (a = x + 1; a < rep->_nsites; ++a)
            {
                ...
                c12 = (c12 == 'Z' && c11 != 'A' && rep->_counts[a].a > 0)
                          ? 'A'
                          : 'Z';
                c12 = (c12 == 'Z' && c11 != 'G' && rep->_counts[a].g > 0)
                          ? 'G'
                          : c12;
                c12 = (c12 == 'Z' && c11 != 'C' && rep->_counts[a].c > 0)
                          ? 'C'
                          : c12;
                c12 = (c12 == 'Z' && c11 != 'T' && rep->_counts[a].t > 0)
                          ? 'T'
                          : c12;
                c12 = (c12 == 'Z' && c11 != '0' && rep->_counts[a].zero > 0)
                          ? '0'
                          : c12;
                c12 = (c12 == 'Z' && c11 != '1' && rep->_counts[a].one > 0)
                          ? '1'
                          : c12;
                ...
                for (b = (flag == false) ? x : a - 1; b < a; ++b)
                    {
                        ...
                        if (states1 == 2 && states2 == 2)
                            {
                                char c21 = 'Z', c22 = 'Z';
                                c21 = (c21 == 'Z' && rep->_counts[b].a > 0)
                                          ? 'A'
                                          : 'Z';
                                c21 = (c21 == 'Z' && rep->_counts[b].g > 0)
                                          ? 'G'
                                          : c21;
                                c21 = (c21 == 'Z' && rep->_counts[b].c > 0)
                                          ? 'C'
                                          : c21;
                                c21 = (c21 == 'Z' && rep->_counts[b].t > 0)
                                          ? 'T'
                                          : c21;
                                c21 = (c21 == 'Z' && rep->_counts[b].zero > 0)
                                          ? '0'
                                          : c21;
                                c21 = (c21 == 'Z' && rep->_counts[b].one > 0)
                                          ? '1'
                                          : c21;
                                ...
                                for (e = 0; e < rep->_nsam; ++e)
                                    {
                                        if (!rep->_haveOutgroup
                                            || (rep->_haveOutgroup
                                                && e != rep->_outgroup))
                                            if (toupper((*rep->_data)[e][a])
                                                    == c11
                                                && toupper((*rep->_data)[e][b])
                                                       == c21)
                                                {
                                                    ++numgametes;
                                                    break;
    ...
    /*!
      \return A vector of statistics related to LD and distance in the sample.
      An empty vector is returned if there are < 2 polymorphic sites in the
      sample.
      See the documentation for Recombination::Disequilibrium for a
      description of the return vector.
      \param mincount a frequency filter.  A polymorphism must be present at
      least \a mincount times in the data
      \note For D and D', the 11 gamete is defined as follows: If no
      outgroup is present, it refers to the genotype of minor alleles at both
      sites.
