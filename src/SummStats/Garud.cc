...
namespace Sequence
{
   ...
   */
  {
	...
	tbb::parallel_for(tbb::blocked_range<std::size_t>(0,uhaps.size()),
			...
			{
			hapcounts[i] = static_cast<double>(std::count(d.begin(),d.end(),vuhaps[i]));
			}
			});
	const double denom  = static_cast<double>(d.size()*(d.size()-1));
	double H1 = tbb::parallel_reduce(tbb::blocked_range<double*>(hapcounts.data(),hapcounts.data()+hapcounts.size()),0.,
			[denom](const tbb::blocked_range<double*> & r,double value) -> double {
				return std::accumulate(r.begin(),r.end(),value,[denom](double s,double c){
	...
	*/
    ...
    double H12 = H1 + 2.*hapcounts[0]*hapcounts[1]/std::pow(double(d.size()),2.);
    double H2H1 = (H1-double(hapcounts[0]*(hapcounts[0]-1))/double(d.size()*(d.size()-1)))/H1;
