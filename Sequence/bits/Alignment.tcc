...
namespace Sequence
{
    namespace Alignment
    {
        template <typename T>
        void
        GetData(std::vector<T> &seqarray, const char *infilename)
        /*!
    ...
    */
        ...
        template <typename T>
        std::istream &
        GetData(std::vector<T> &seqarray, std::istream &input_stream)
        /*!
      Read objects of type T and put them into the vector seqarray.
      Note that seqarray is not const, so that's where the data go.
    ...
    */
        ...
        template <typename T>
        std::istream &
        ReadNObjects(std::vector<T> &seqarray, const unsigned &n,
    ...
    */
        ...
        template <typename T>
        bool
        Gapped(const std::vector<T> &data)
        /*!
    ...
    */
        {
            for (typename std::vector<T>::size_type i = 0; i < data.size();
                 ++i)
                //iterate over sequences
                {
                    if (data[i].seq.find('-') != std::string::npos)
                        return true;
                }
            return false;
        }
        ...
        template <typename T>
        bool
        IsAlignment(const std::vector<T> &data)
        /*!
    ...
    */
        {
            for (typename std::vector<T>::size_type i = 0; i < data.size();
                 ++i)
                {
                    if (data[i].seq.length() != data[0].seq.length())
                        {
                            return 0;
                        }
                }
        ...
        bool
        validForPolyAnalysis(Iterator beg, Iterator end)
        /*!
    ...
    */
        {
            while (beg < end)
                {
                    if (std::find_if(beg->seq.begin(), beg->seq.end(),
        ...
        template <typename T>
        unsigned
        UnGappedLength(const std::vector<T> &data)
    ...
    */
        {
            ...
            for (size_t j = 0; j < data[0].seq.length(); ++j)
                {
                    bool site_gapped = 0;
    ...
    */
        {
            ...
            size_t length = data[0].seq.length();
            std::vector<std::string> ungapped_sequences(data.size());
            bool site_is_gapped;

            for (i = 0; i < length; ++i)
                { //iterate over sites
                    for (j = 0, site_is_gapped = 0; j < datasize; ++j)
                        {
                            if (data[j].seq[i] == '-')
                                {
                                    site_is_gapped = 1;
                    ...
                    if (!(site_is_gapped))
                        {
                            for (j = 0; j != data.size(); ++j)
                                ungapped_sequences[j] += data[j].seq[i];
                        }
                }
            ...
            for (j = 0; j != datasize; ++j)
                {
                    data[j].seq = std::move(ungapped_sequences[j]);
                }
        }
        ...
        template <typename T>
        void
        RemoveTerminalGaps(std::vector<T> &data)
        /*!
      Remove all gapped sites from the ends of the alignment,
      up until the name site on either side that is ungapped.
      \param data vector<T> to modify
    */
        {
            size_t i, j,
                length
                = data[0].seq.length(); //how much we have to iterate over
            ...
            for (i = length - 1;
                 i < data[0].seq.length() && exit_condition == false; --i)
                {
                    for (numUngapped = 0, j = 0; j != data.size(); ++j)
                        {
                            if (data[j].seq[i] != '-')
            ...
            for (j = 0; j != data.size(); ++j)
                trimmed_sequences.push_back(
                    data[j].seq.substr(leftmost, offset));

            //now, redo the seq array for the current object
            for (j = 0; j != data.size(); ++j)
                {
                    //Probably can't hurt...
                    data[j] = std::move(T(data[j].name, trimmed_sequences[j]));
                }
        }

        template <typename T>
        void
        RemoveFixedOutgroupInsertions(std::vector<T> &data,
                                      const unsigned &ref)
        /*!
    ...
    */
        {
            ...
            size_t s = data[0].seq.length() - 1, max = s + 1;
            for (size_t i = 0; i < max; --s, ++i)
                {
                    unsigned ngap = 0;
                    for (size_t ind = 0; ind < data.size(); ++ind)
                        {
                            if (ind != ref && data[ind].seq[s] == '-')
                                {
                                    ngap++;
                                }
                        }
        ...
        template <typename T>
        std::vector<T>
        Trim(const std::vector<T> &data, const std::vector<int> &sites)
    ...
    */
        {
            ...
            if (sites.empty())
                {
                    throw std::runtime_error("Sequence::Alignment::Trim(): "
                                             "empty vector of positions "
                                             "passed to function");
                }
            if (numIntervals % 2 != 0)
                {
                    throw std::runtime_error("Sequence::Alignment::Trim(): "
                                             "odd number of positions passed");
                }
            ...
            for (i = 0; i < numIntervals; i += 2)
                {
                    ...
                    for (j = 0; j < numseqs; ++j)
                        {
                            trimmedTemp[j]
                                += data[j].seq.substr(start, stop - start + 1);
                        }
                }

            for (i = 0; i < numseqs; ++i)
                {
                    trimmedData[i].name = data[i].name;
                    trimmedData[i].seq = std::move(trimmedTemp[i]);
                }
        ...
        template <typename T>
        std::vector<T>
        TrimComplement(const std::vector<T> &data,
    ...
    */
        {
            ...
            if (sites.empty())
                {
                    throw std::runtime_error(
                        "Sequence::Alignment::TrimComplement(): empty vector "
                        "of positions passed to function");
                }
            if (numIntervals % 2 != 0)
                {
                    throw std::runtime_error("Sequence::Alignment::"
                                             "TrimComplement(): odd numer of "
                                             "positions passed to function");
                }
            ...
            for (j = 0; j < numseqs; ++j)
                {
                    trimmedTemp[j] += data[j].seq.substr(lastval);
                }
